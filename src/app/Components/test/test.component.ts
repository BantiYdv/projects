...
export class TestComponent {
  ...
  applyFilter() {
    this.filteredEmployeeData = this.EmployeeData.filter(
  ...
  applyFilterDesignation() {
    this.filteredEmployeeData = this.EmployeeData.filter(
      (item: { designation: string }) =>
        item.designation
          .toLowerCase()
          .includes(this.searchdesignation.toLowerCase())
    );
  ...
  FilterRole() {
    this.fillterAllRoleData = this.AllRoleData.filter(
      (item: { name: string }) =>
  ...
  addDocument() {
    if (this.newDocument.documentName.trim() !== '') {
      this.offerLetter.selectedDocuments.push({
...
this.getAnnouncementForToday();
    ...
    this.ratingMonth = this.saveDataRating.monthAndYear = new Date().toISOString().split('T')[0].slice(0, -3);
    const currentDate = new Date();
    currentDate.setMonth(currentDate.getMonth() - 1);

    this.reviewMonth = currentDate.toISOString().split('T')[0].slice(0, -3);
  ...
  openEmployee(): void {
    ...
    if (
      '/employee' === this.router.url ||
      '/reports' === this.router.url ||
      '/performance' === this.router.url ||
      '/viewPerformance' === this.router.url
    ) {
    ...
    if ('/teamleave' === this.router.url) {
      this.testService.getTeamLeaveData().subscribe(
        (response: any[]) => {
          ...
          const sortedData = reversedData.sort(
            (a, b) =>
          ...
          if (this.searchTeamLeave != '' || this.selectedMonthTeamLeave != '') {
            ...
            const currentDateData = response
              .filter((item: { fromDate: string }) =>
                item.fromDate.includes(this.viewAllTodayTeamLeave())
              )
              .sort(
                (a, b) =>
      ...
      this.testService.getTeamWfhData().subscribe(
        (response: any) => {
          ...
          if (this.searchTeamWfh != '' || this.selectedMonthTeamWFH != '') {
            ...
            this.fillterTeamWfhData = response.filter(
              (item: { fromdateWfh: string }) =>
                item.fromdateWfh.includes(this.viewAllTodayTeamWFH())
            );
            this.TeamWfhData = response.filter(
              (item: { fromdateWfh: string }) =>
                item.fromdateWfh.includes(this.viewAllTodayTeamWFH())
            );
          }
  ...
  AddPermission(id: any) {
    if (id) {
      this.testService
        .AddPermissionName(id, this.selectedPermissions)
        .subscribe(
          (response) => {
            Swal.fire({
              icon: 'success',
  ...
  deletePermission(id: any): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.testService
          .RemovePermission(id, this.selectedPermissions)
          .subscribe(
            () => {
  ...
  getAllEmployees() {
    ...
    this.testService.getEmployeeListExcel(headers).subscribe(
  ...
  getAllEmployeesLeave() {
    this.testService.getEmployeeLeaveExcel(this.searchEmployee).subscribe(
      (response: HttpResponse<Blob>) => {
        if (response.body) {
          const contentDisposition = response.headers.get(
            'content-disposition'
  ...
  getAllEmployeesAtt() {
    this.testService.getEmployeeAttExcel(this.searchEmployee).subscribe(
      (response: HttpResponse<Blob>) => {
        // Check if the response body is not null
        if (response.body) {
          const contentDisposition = response.headers.get(
  ...
  getAllEmployeesWfh() {
    this.testService.getEmployeeWfhExcel(this.searchEmployee).subscribe(
      (response: HttpResponse<Blob>) => {
        // Check if the response body is not null
        if (response.body) {
          const contentDisposition = response.headers.get(
  ...
  updateEndDate(): void {
    ...
    if (this.leaveForm.get('toDate').value === '') {
      ...
      this.leaveForm
        .get('toDate')
        .setValue(endDate.toISOString().substring(0, 10)); // Set end date in ISO format (YYYY-MM-DD)
    }
  ...
  padZero(num: number): string {
    //The padZero() method is another helper function to add a leading zero to single-digit numbers for proper date formatting.
      ...
      this.testService.getAttendance().subscribe(
        (response: any) => {
          ...
          if (this.searchatt != '' || this.selectedMonthAtt != '') {
            ...
            this.fillterattData = response.filter(
              (item: { checkDate: string }) =>
  ...
  onWFHSubmit() {
    ...
    this.testService.applyWfh(this.wfhForm.value).subscribe(
      () => {
        ...
        Swal.fire(
          'Success!',
          'Work From Home Applied successfully, Waiting for response!',
          'success'
        );
        // this.wfhForm.reset()
      ...
      this.testService.getAllAttendance().subscribe(
        (response: any) => {
          ...
          if (this.searchAllatt != '' || this.selectedMonth != '') {
            ...
            this.fillterAllattData = response.filter(
              (item: { checkDate: string }) =>
      ...
      this.testService.getAllWfhData().subscribe(
        (response: any) => {
          ...
          if (this.searchAllWfh != '' || this.selectedMonthAllWFH != '') {
            ...
            this.fillterAllWfhData = response.filter(
              (item: { fromdateWfh: string }) =>
      ...
      this.testService.getAllLeave().subscribe(
        (response: any) => {
          ...
          if (this.searchAllLeave != '' || this.selectedMonthAllLeave != '') {
            ...
            this.fillterAllLeaveData = response.filter(
  ...
  openAddPermission(item: any) {
    ...
    this.selectedRole = item;
    // Check if "NO_ACCESS" is in the permissions list
    const hasNoAccess = item.permissions.some(
      (permission: { permissionName: any }) =>
        permission.permissionName === 'NO_ACCESS'
    );
    ...
    if (hasNoAccess) {
      ...
      // If "NO_ACCESS" is not present, keep the existing permissions
      this.selectedPermissions = item.permissions.map(
  ...
  removePermission(permission: any) {
    ...
    const optionIndex = this.availablePermissionOptions.findIndex(
      (opt) => opt === permission
    );
    if (optionIndex !== -1) {
      this.form
        ...
        .patchValue([
          ...this.form
            .get('url')
            .value.filter((opt: any) => opt !== permission),
        ]);
    }
  }
  ...
  onPermissionSelectionChange() {
    if (this.selectedPermissions.includes('NO_ACCESS')) {
      ...
      // If "ALL_ACCESS" is selected, set selectedPermissions to all available options except "NO_ACCESS"
      this.selectedPermissions = this.availablePermissionOptions.filter(
        (option) => option !== 'NO_ACCESS'
      );
    }
  }
  ...
  isOptionDisabled(option: string): boolean {
    if (
      (this.selectedPermissions.includes('NO_ACCESS') &&
        option !== 'NO_ACCESS') ||
      (this.selectedPermissions.includes('ALL_ACCESS') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('ALL_ACCESS') &&
        option !== 'ALL_ACCESS') ||
      (this.selectedPermissions.includes('ALL_EMPLOYEES_DATA') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('NEW_REGISTRATION') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('ALL_EMPLOYEES_ATTENDANCE') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('LEAVE_SHOW_TEAMLEAD') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('WFH_SHOW_TEAMLEAD') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('ALL_WFH_EMPLOYEES') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('VIEW_ALL_LEAVE') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('ADD_HOLIDAY') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('ADD_BIRTHDAY') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('CREATE_ROLE') &&
        option === 'NO_ACCESS') ||
      // (this.selectedPermissions.includes('DOWNLOAD_REPORTS') && option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('DOWNLOAD_EMPLOYEE_REPORTS') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('DOWNLOAD_WORKFROMHOME_REPORTS') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('DOWNLOAD_LEAVES_REPORTS') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('DOWNLOAD_ATTENDANCE_REPORTS') &&
        option === 'NO_ACCESS') ||
      (this.selectedPermissions.includes('UPLOAD_LEAVE_POLICY') &&
        option === 'NO_ACCESS')
    ) {
  ...
  openDeletePermission(item: any) {
    this.selectedRole = item;
    this.availablePermissions = item.permissions.map(
      (permission: { permissionName: any }) => permission.permissionName
    );
  ...
  getTotalPagesRole(): number {
    return Math.ceil(this.fillterAllRoleData.length / this.RoleperPage);
  }
  ...
  getTotalPagesEmp(): number {
    return Math.ceil(this.filteredEmployeeData.length / this.employeeperPage);
  }
  ...
  getPaginatedTeamLeaveData(): any[] {
    const startIndex =
      (this.currentTeamLeavePage - 1) * this.TeamLeavePageperPage;
  ...
  getTotalPagesTeamWfh(): number {
    return Math.ceil(this.fillterTeamWfhData.length / this.TeamWfhPageperPage);
  }
    ...
    if ('/viewPayroll' === this.router.url) {
      this.testService.getEmployeeList().subscribe(
        (response: any) => {
          ...
          this.employeeNames = response.map(
            (employee: { firstname: any; lastname: any }) => ({
  ...
  emailIds() {
    if ('/viewPayroll' === this.router.url) {
      this.testService.getEmployeeList().subscribe(
        (response: any) => {
          // Assuming the response is an array
          this.emailId = response.map((email: { emailid: any }) => ({
  ...
  sortByToDate(): void {
    this.sortTDate = this.sortTDate === 'asc' ? 'desc' : 'asc';
  ...
  sortByFromDate(): void {
    this.sortFDate = this.sortFDate === 'asc' ? 'desc' : 'asc';
  ...
  FilterTeamLeave() {
    if (this.searchTeamLeave) {
      this.fillterTeamLeaveData = this.TeamLeaveData.filter(
        (item: { firstname: string; lastname: string }) =>
          item.firstname
            .toLowerCase()
            .includes(this.searchTeamLeave.toLowerCase()) ||
          item.lastname
            .toLowerCase()
            .includes(this.searchTeamLeave.toLowerCase()) ||
      ...
      this.fillterTeamLeaveData = this.TeamLeaveData.filter(
        (item: { fromDate: string }) =>
          item.fromDate
            .toLowerCase()
            .includes(this.selectedMonthTeamLeave.toLowerCase())
      );
    }
  }
  ...
  FilterTeamWfh() {
    if (this.searchTeamWfh) {
      this.fillterTeamWfhData = this.TeamWfhData.filter(
        (item: { firstname: string; lastname: string }) =>
          item.firstname
            .toLowerCase()
            .includes(this.searchTeamWfh.toLowerCase()) ||
          item.lastname
            .toLowerCase()
            .includes(this.searchTeamWfh.toLowerCase()) ||
          (item.firstname + ' ' + item.lastname)
      ...
      this.fillterTeamWfhData = this.TeamWfhData.filter(
        (item: { fromdateWfh: string }) =>
          item.fromdateWfh
            .toLowerCase()
            .includes(this.selectedMonthTeamWFH.toLowerCase())
      );
    }
  }
  onSearchTeamWFH() {
  ...
  FilterLeave() {
    this.fillterLeaveData = this.leaveAdminData.filter(
      (item: { firstname: string; lastname: string }) =>
  ...
  getTotalPagesLeave(): number {
    return Math.ceil(this.fillterLeaveData.length / this.LeavePageperPage);
  }
  ...
  FilterWfh() {
    this.fillterWfhData = this.wfhData.filter(
      (item: { firstname: string; lastname: string }) =>
  ...
  getTotalPagesWfh(): number {
    return Math.ceil(this.fillterWfhData.length / this.WfhPageperPage);
  }
  ...
  FilterAllatt() {
    if (this.searchAllatt) {
      this.fillterAllattData = this.AllAttData.filter(
        (item: { firstname: string; lastname: string }) =>
          item.firstname
            .toLowerCase()
            .includes(this.searchAllatt.toLowerCase()) ||
          item.lastname
            .toLowerCase()
            .includes(this.searchAllatt.toLowerCase()) ||
          (item.firstname + ' ' + item.lastname)
      ...
      this.fillterAllattData = this.AllAttData.filter(
        (item: { checkDate: string }) =>
          item.checkDate
            .toLowerCase()
            .includes(this.selectedMonth.toLowerCase())
      );
    }
  }
  ...
  getTotalPagesAllatt(): number {
    return Math.ceil(this.fillterAllattData.length / this.AllattperPage);
  }
  ...
  sortPositionsBySalary(): void {
    this.sortOrderPositionSalary =
      this.sortOrderPositionSalary === 'asc' ? 'desc' : 'asc';
  ...
  leaveRules = [
    {
      label: 'Sick Leave Carryforward',
      name: 'sickLeaveCarryforward',
      value_y: 'yes',
      value_n: 'no',
    },
    {
      label: 'Sick Leave Encashment',
      name: 'sickLeaveEncashment',
      value_y: 'yes',
      value_n: 'no',
    },
    {
      label: 'Sick Leave Expire',
      name: 'sickLeaveExpire',
      value_y: 'yes',
      value_n: 'no',
    },
    {
      label: 'Casual Leave Carryforward',
      name: 'casualLeaveCarryforward',
      value_y: 'yes',
      value_n: 'no',
    },
    {
      label: 'Casual Leave Encashment',
      name: 'casualLeaveEncashment',
      value_y: 'yes',
      value_n: 'no',
    },
    {
      label: 'Casual Leave Expire',
      name: 'casualLeaveExpire',
      value_y: 'yes',
      value_n: 'no',
    },
  ...
  viewPositionName() {
    this.testService.viewPositionName().subscribe((response) => {
  ...
  addInterviewRecruitment() {
    this.testService
      .addInterview(this.interview, this.selectedFileName)
      .subscribe(
        (response: any) => {
          console.log('addPositionRecruitment success', response);
          Swal.fire({
  ...
  putStatusPosition(positionId: any, positionStatus: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.testService
          .updatePositionstatus(positionId, positionStatus)
          .subscribe(
            (response) => {
  ...
  updateConfirmationStatus(confirmationId: any, confirmationStatus: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.testService
          .updateConfirmationStatus(confirmationId, confirmationStatus)
          .subscribe(
            (response) => {
              console.log('Status updated successfully:', response);
  ...
  addInterviewUpdateRecruitment(
    id: any,
    interview: any,
    selectedFileName: any
  ) {
    this.testService
      .updateInterviewById(id, interview, selectedFileName)
      .subscribe(
        (response) => {
          // this.positionsDataById = response;
          Swal.fire({
  ...
  FilterShift() {
    this.fillterShiftData = this.allShiftData.filter(
      (item: { checkInTime: string; checkOutTime: string }) =>
        item.checkInTime
          .toLowerCase()
          .includes(this.searchShift.toLowerCase()) ||
  ...
  editShift(itemId: number) {
    ...
    this.selectedItem = this.fillterShiftData.find(
  ...
  incrementNoticePeriod(): void {
    console.log('this.noticePeriod', this.noticePeriod);
    this.noticePeriod++;
  }
  ...
  FilterTodayAtt() {
    this.fillterTodayattData = this.TodayAttData.filter(
      (item: { firstname: string; lastname: string }) =>
        item.firstname
          .toLowerCase()
          .includes(this.searchTodayAtt.toLowerCase()) ||
  ...
  getTotalPagesTodayatt(): number {
    return Math.ceil(this.fillterTodayattData.length / this.todayAttPerPage);
  }
  ...
  FilterTodayWfh() {
    this.fillterTodayWfhData = this.TodayWfhData.filter(
      (item: { firstname: string; lastname: string }) =>
        item.firstname
          .toLowerCase()
          .includes(this.searchTodayWfh.toLowerCase()) ||
  ...
  getTotalPagesTodayWfh(): number {
    return Math.ceil(this.fillterTodayWfhData.length / this.todayWfhPerPage);
  }
  ...
  FilterTodaySick() {
    this.fillterTodaySickLeaveData = this.TodaySickLeaveData.filter(
      (item: { firstname: string; lastname: string }) =>
        item.firstname
          .toLowerCase()
          .includes(this.searchTodaySick.toLowerCase()) ||
  ...
  FilterTodayCasual() {
    this.fillterTodayCasualLeaveData = this.TodayCasualLeaveData.filter(
      (item: { firstname: string; lastname: string }) =>
        item.firstname
          .toLowerCase()
          .includes(this.searchTodayCasual.toLowerCase()) ||
        item.lastname
          .toLowerCase()
          .includes(this.searchTodayCasual.toLowerCase())
    );
  ...
  getPaginatedTodayCasualData(): any[] {
    const startIndex =
      (this.currentTodayCasualPage - 1) * this.todayCasualPerPage;
  ...
  getPageNumbersTodayCasual(): number[] {
    ...
    if (totalPages <= maxPagesToShow) {
      ...
      if (this.currentTodayCasualPage <= halfMaxPagesToShow + 1) {
        ...
        this.currentTodayCasualPage + halfMaxPagesToShow >=
        totalPages
      ) {
        startPage = totalPages - maxPagesToShow + 1;
        endPage = totalPages;
  ...
  FilterTodayAbsent() {
    this.fillterTodayAbsentLeaveData = this.TodayAbsentLeaveData.filter(
      (item: { firstname: string; lastname: string }) =>
        item.firstname
          .toLowerCase()
          .includes(this.searchTodayAbsent.toLowerCase()) ||
        item.lastname
          .toLowerCase()
          .includes(this.searchTodayAbsent.toLowerCase())
    );
  ...
  getPaginatedTodayAbsentData(): any[] {
    const startIndex =
      (this.currentTodayAbsentPage - 1) * this.todayAbsentPerPage;
  ...
  getPageNumbersTodayAbsent(): number[] {
    ...
    if (totalPages <= maxPagesToShow) {
      ...
      if (this.currentTodayAbsentPage <= halfMaxPagesToShow + 1) {
        ...
        this.currentTodayAbsentPage + halfMaxPagesToShow >=
        totalPages
      ) {
        startPage = totalPages - maxPagesToShow + 1;
        endPage = totalPages;
    ...
    if ('/todayAbsentLeave' === this.router.url) {
      ...
      this.testService.getTodayAbsent().subscribe(
        (response: any) => {
          ...
          console.log(
            'today Absent leave?????',
            this.fillterTodayAbsentLeaveData
          );
        },
        (error) => {
          Swal.fire('Error', error.error, 'error');
  ...
  FilterFullTimeEmp() {
    this.fillterFullTimeEmpData = this.FullTimeEmpData.filter(
      (item: { firstname: string; lastname: string; designation: string }) =>
        item.firstname
          .toLowerCase()
          .includes(this.searchFullTimeEmp.toLowerCase()) ||
        item.lastname
          .toLowerCase()
          .includes(this.searchFullTimeEmp.toLowerCase()) ||
        item.designation
  ...
  getPaginatedFullTimeEmpData(): any[] {
    const startIndex =
      (this.currentFullTimeEmpPage - 1) * this.FullTimeEmpPerPage;
  ...
  getPageNumbersFullTimeEmp(): number[] {
    ...
    if (totalPages <= maxPagesToShow) {
      ...
      if (this.currentFullTimeEmpPage <= halfMaxPagesToShow + 1) {
        ...
        this.currentFullTimeEmpPage + halfMaxPagesToShow >=
        totalPages
      ) {
        startPage = totalPages - maxPagesToShow + 1;
        endPage = totalPages;
  ...
  FilterPartTimeEmp() {
    this.fillterPartTimeEmpData = this.PartTimeEmpData.filter(
      (item: { firstname: string; lastname: string }) =>
        item.firstname
          .toLowerCase()
          .includes(this.searchPartTimeEmp.toLowerCase()) ||
        item.lastname
          .toLowerCase()
          .includes(this.searchPartTimeEmp.toLowerCase())
    );
  ...
  getPaginatedPartTimeEmpData(): any[] {
    const startIndex =
      (this.currentPartTimeEmpPage - 1) * this.PartTimeEmpPerPage;
  ...
  getPageNumbersPartTimeEmp(): number[] {
    ...
    if (totalPages <= maxPagesToShow) {
      ...
      if (this.currentPartTimeEmpPage <= halfMaxPagesToShow + 1) {
        ...
        this.currentPartTimeEmpPage + halfMaxPagesToShow >=
        totalPages
      ) {
        startPage = totalPages - maxPagesToShow + 1;
        endPage = totalPages;
  ...
  FilterinternEmp() {
    this.fillterinternEmpData = this.internEmpData.filter(
      (item: { firstname: string; lastname: string }) =>
        item.firstname
          .toLowerCase()
          .includes(this.searchinternEmp.toLowerCase()) ||
  ...
  getTotalPagesinternEmp(): number {
    return Math.ceil(this.fillterinternEmpData.length / this.internEmpPerPage);
  }
  ...
  Filteratt() {
    if (this.searchatt) {
      this.fillterattData = this.AttData.filter(
        (item: { firstname: string; lastname: string }) =>
      ...
      this.fillterattData = this.AttData.filter((item: { checkDate: string }) =>
        item.checkDate
          .toLowerCase()
          .includes(this.selectedMonthAtt.toLowerCase())
      );
    }
  }
  ...
  nextPageatt(): void {
    const totalPages = Math.ceil(this.fillterattData.length / this.attperPage);
  ...
  getTotalPagesatt(): number {
    return Math.ceil(this.fillterattData.length / this.attperPage);
  }
  ...
  FilterAllWfh() {
    if (this.searchAllWfh) {
      this.fillterAllWfhData = this.AllwfhData.filter(
        (item: { firstname: string; lastname: string }) =>
          item.firstname
            .toLowerCase()
            .includes(this.searchAllWfh.toLowerCase()) ||
          item.lastname
            .toLowerCase()
            .includes(this.searchAllWfh.toLowerCase()) ||
          (item.firstname + ' ' + item.lastname)
      ...
      this.fillterAllWfhData = this.AllwfhData.filter(
        (item: { fromdateWfh: string }) =>
          item.fromdateWfh
            .toLowerCase()
            .includes(this.selectedMonthAllWFH.toLowerCase())
      );
    }
  }
  onSearchAllWFH() {
  ...
  getTotalPagesAllWfh(): number {
    return Math.ceil(this.fillterAllWfhData.length / this.AllWfhPageperPage);
  }
  ...
  FilterAllLeave() {
    if (this.searchAllLeave) {
      this.fillterAllLeaveData = this.leaveAllData.filter(
        (item: { firstname: string; lastname: string }) =>
          item.firstname
            .toLowerCase()
            .includes(this.searchAllLeave.toLowerCase()) ||
          item.lastname
            .toLowerCase()
            .includes(this.searchAllLeave.toLowerCase()) ||
          (item.firstname + ' ' + item.lastname)
      ...
      this.fillterAllLeaveData = this.leaveAllData.filter(
        (item: { fromDate: string }) =>
          item.fromDate
            .toLowerCase()
            .includes(this.selectedMonthAllLeave.toLowerCase())
      );
    }
  }
  onSearchAllLeave() {
  ...
  getPaginatedAllLeaveData(): any[] {
    const startIndex =
      (this.currentAllLeavePage - 1) * this.AllLeavePageperPage;
  ...
  FilterHoliday() {
    this.fillterGetHoliday = this.getHolidayData.filter(
      (item: { holiDayReason: string }) =>
        item.holiDayReason
          .toLowerCase()
          .includes(this.searchHoliday.toLowerCase())
    );
  }
  ...
  getTotalPagesHoliday(): number {
    return Math.ceil(this.fillterGetHoliday.length / this.holidayPerPage);
  }
  ...
  FilterAbsent() {
    this.fillterGetAbsent = this.getAbsentData.filter(
      (item: { firstname: string; lastname: string }) =>
        item.firstname
          .toLowerCase()
          .includes(this.searchAbsent.toLowerCase()) ||
  ...
  getTotalPagesAbsent(): number {
    return Math.ceil(this.fillterGetAbsent.length / this.absentPerPage);
  }
  ...
  FilterLast7DaysPresent() {
    this.fillterLast7DaysPresentData = this.presentLast7Days.filter(
      (item: { name: string }) =>
        item.name
          .toLowerCase()
          .includes(this.searchLast7DaysPresent.toLowerCase())
    );
  }

  getPaginatedLast7DaysPresentData(): any[] {
    const startIndex =
      (this.currentLast7DaysPresentPage - 1) * this.Last7DaysPresentPerPage;
    ...
    this.dashboardService.presentUsersLast7Days().subscribe(
      (response: any) => {
        ...
        const filteredData = response.filter(
          (data: { date: string | null }) => data.date === this.clickedDate
        );
  ...
  FilterLast7DaysLateArrival() {
    this.fillterLast7DaysLateArrivalData = this.totalCheckedInLate.filter(
      (item: { name: string }) =>
        item.name
          .toLowerCase()
          .includes(this.searchLast7DaysLateArrival.toLowerCase())
    );
  }

  getPaginatedLast7DaysLateArrivalData(): any[] {
    const startIndex =
      (this.currentLast7DaysLateArrivalPage - 1) *
      this.Last7DaysLateArrivalPerPage;
  ...
  getPageNumbersLast7DaysLateArrival(): number[] {
    const totalPages = Math.ceil(
      this.fillterLast7DaysLateArrivalData.length /
        this.Last7DaysLateArrivalPerPage
    );
    return Array.from({ length: totalPages }, (_, index) => index + 1);
  }
  ...
  changePageLast7DaysLateArrival(pageNumber: number): void {
    if (
      pageNumber >= 1 &&
      pageNumber <= this.getTotalPagesLast7DaysLateArrival()
    ) {
    ...
    this.dashboardService.checkedInLate().subscribe(
      (response: any) => {
        ...
        if (filteredData.length > 0) {
          ...
          console.log(
            'View late arrival Last 7Days>>>>>>',
            this.totalCheckedInLate
          );
        } else {
          console.log('No data available for the selected date');
          // Handle the case when no data is available for the selected date
  ...
  FilterLast7DaysEarlyDeparture() {
    this.fillterLast7DaysEarlyDepartureData = this.totalCheckOutEarly.filter(
      (item: { name: string }) =>
        item.name
          .toLowerCase()
          .includes(this.searchLast7DaysEarlyDeparture.toLowerCase())
    );
  }

  getPaginatedLast7DaysEarlyDepartureData(): any[] {
    const startIndex =
      (this.currentLast7DaysEarlyDeparturePage - 1) *
      this.Last7DaysEarlyDeparturePerPage;
  ...
  getPageNumbersLast7DaysEarlyDeparture(): number[] {
    const totalPages = Math.ceil(
      this.fillterLast7DaysEarlyDepartureData.length /
        this.Last7DaysEarlyDeparturePerPage
    );
    return Array.from({ length: totalPages }, (_, index) => index + 1);
  }
  ...
  changePageLast7DaysEarlyDeparture(pageNumber: number): void {
    if (
      pageNumber >= 1 &&
      pageNumber <= this.getTotalPagesLast7DaysEarlyDeparture()
    ) {
    ...
    this.dashboardService.checkOutEarly().subscribe(
      (response: any) => {
        ...
        if (filteredData.length > 0) {
          ...
          console.log(
            'View early departure Last 7Days>>>>>>',
  ...
  FilterBirthday() {
    this.fillterGetBirthday = this.getBirthdayData.filter(
      (item: { name: string }) =>
  ...
  getTotalPagesBirthday(): number {
    return Math.ceil(this.fillterGetBirthday.length / this.birthdayPerPage);
  }
  ...
  FilterPayroll() {
    this.fillterGetPayroll = this.getPayrollData.filter(
      (payroll: { employeeName: string }) =>
        payroll.employeeName
          .toLowerCase()
          .includes(this.searchPayroll.toLowerCase())
    );
  }

  applyFilterPayroll() {
    this.fillterGetPayroll = this.getPayrollData.filter(
      (payroll: { emailId: string }) =>
        payroll.emailId
          .toLowerCase()
          .includes(this.searchPayrollEmail.toLowerCase())
    );
  }

  applyFilterPayrollDesignation() {
    this.fillterGetPayroll = this.getPayrollData.filter(
      (payroll: { designation: string }) =>
        payroll.designation
          .toLowerCase()
          .includes(this.searchPayrolldesignation.toLowerCase())
    );
  ...
  getTotalPagesPayroll(): number {
    return Math.ceil(this.fillterGetPayroll.length / this.payrollPerPage);
  }
  ...
  excelSalarySheet() {
    ...
    this.testService.downloadSalaryPayrollExcel(headers).subscribe(
      (response: HttpResponse<Blob>) => {
        // Check if the response body is not null
        if (response.body) {
          const contentDisposition = response.headers.get(
  ...
  generateOfferLetter() {
    this.offerLetter.timeDurationOfConfirmation =
      this.offerLetter.confirmationDuration1 +
      ' ' +
      this.offerLetter.confirmationDuration2;
  ...
  FilterAppraisal() {
    this.fillterAppraisal = this.getAppraisalData.filter(
      (item: { employeeName: string }) =>
        item.employeeName
          .toLowerCase()
          .includes(this.searchAppraisal.toLowerCase())
    );
  }
  ...
  getTotalPagesAppraisal(): number {
    return Math.ceil(this.fillterAppraisal.length / this.AppraisalperPage);
  }
  ...
  FilterPromotion() {
    this.fillterPromotion = this.getPromotionData.filter(
      (item: { employeeName: string }) =>
        item.employeeName
          .toLowerCase()
          .includes(this.searchPromotion.toLowerCase())
    );
  }
  ...
  getTotalPagesPromotion(): number {
    return Math.ceil(this.fillterPromotion.length / this.PromotionperPage);
  }
  ...
  FilterDemotion() {
    this.fillterDemotion = this.getDemotionData.filter(
      (item: { employeeName: string }) =>
        item.employeeName
          .toLowerCase()
          .includes(this.searchDemotion.toLowerCase())
    );
  }
  ...
  getTotalPagesDemotion(): number {
    return Math.ceil(this.fillterDemotion.length / this.DemotionperPage);
  }
  ...
  getPageNumbersPIP(): number[] {
    const totalPages = Math.ceil(this.fillterPIP.length / this.PIPPage);
    return Array.from({ length: totalPages }, (_, index) => index + 1);
  }
  ...
  getTotalPagesPIP(): number {
    return Math.ceil(this.fillterPIP.length / this.PIPPage);
  }
  ...
  getOverallReviewForFebruary(reviews: any[]) {
    ...
    const februaryReviews = reviews.filter(
      (review) => review.monthAndYear === this.reviewMonth
    );
  ...
  FilterReviewAll() {
    this.fillterReviewAll = this.getReviewAllData.filter(
      (item: { firstname: string; lastname: string }) =>
        item.firstname
          .toLowerCase()
          .includes(this.searchReviewAll.toLowerCase()) ||
        item.lastname
          .toLowerCase()
          .includes(this.searchReviewAll.toLowerCase()) ||
        (item.firstname + ' ' + item.lastname)
  ...
  getTotalPagesReviewAll(): number {
    return Math.ceil(this.fillterReviewAll.length / this.ReviewAllperPage);
  }
  ...
  FilterAllPrbation() {
    this.fillterAllPrbation = this.getAllPrbationData.filter(
      (item: { firstname: string; lastname: string }) =>
        item.firstname
          .toLowerCase()
          .includes(this.searchAllPrbation.toLowerCase()) ||
        item.lastname
          .toLowerCase()
          .includes(this.searchAllPrbation.toLowerCase()) ||
        (item.firstname + ' ' + item.lastname)
  ...
  getPaginatedAllPrbation(): any[] {
    const startIndex =
      (this.currentAllPrbationPage - 1) * this.AllPrbationperPage;
  ...
  getTotalPagesAllPrbation(): number {
    return Math.ceil(this.fillterAllPrbation.length / this.AllPrbationperPage);
  }
  // pagination and search for view probation end
...
viewEmployeeRating= {reviews: [ {accuracyOfWork: 0,dedication: 0,performance: 0,teamWork: 0,abilityToLearn: 0,attendance: 0,punctuality: 0,workHabits: 0,responsiveness: 0}]};
username: any;
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      this.RegisterAndUpdate.fetchData(id).subscribe(
        (r: any) => {
          ...
          // console.warn('r.reviews',r.reviews[0].accuracyOfWork);
  
          // this.saveDataProbation.id = r.id;reviewMonth
          this.viewEmployeeRating.reviews = r.reviews.filter((review: { monthAndYear: any; }) => this.reviewMonth == review.monthAndYear);
          console.warn('1111111');
   this.viewEmployeePromotion();
   this.viewEmployeeDemotion();
   this.viewEmployeeAppraisal();
   this.viewEmployeePIP();
  ...
  viewEmployeeDemotion(){
    this.testService.viewEmployeeDemotion(this.viewProbationData.username).subscribe(
      (r:any) => {
        console.warn('viewEmployeeDemotionData',r);
        this.viewEmployeeDemotionData = r.data;
  ...
  viewEmployeePIP(){
    this.testService.viewEmployeePIP(this.viewProbationData.username).subscribe(
      (r:any) => {
        console.warn('viewEmployeePIPData',r);
        this.viewEmployeePIPData = r.data;
  ...
  viewEmployeePromotion(){
    this.testService.viewEmployeePromotion(this.viewProbationData.username).subscribe(
      (r:any) => {
  ...
  viewEmployeeAppraisal(){
    this.testService.viewEmployeeAppraisal(this.viewProbationData.username).subscribe(
      (r:any) => {
...
//Announcement code start
  getAllAnnouncementData: any;
  fillterAllAnnouncement: any;
...
addAnnouncement() {
  console.warn('add Announcement', this.addAnnouncementData);
  this.testService.addAnnouncement(this.addAnnouncementData).subscribe(
    (r: any) => {
      console.log('r', r);
      Swal.fire({
        icon: 'success',
     ...
     this.router.navigate(['/viewAnnouncement']);
      this.loginService.showTable('viewAnnouncement');
      this.viewAllAnnouncement();
    },
    (e: any) => {
      console.error('e', e);
      Swal.fire({
        icon: 'error',
        title: 'Save Failed',
        text: 'There was an error while saving the Announcement. Please try again.',
      });
    }
  );
}
...
// view All Announcement start
viewAllAnnouncement(){
  console.log("befor", this.router.url);
  if ('/viewAnnouncement' === this.router.url ){
    console.log("after", this.router.url);
    this.testService.viewAllAnnouncement().subscribe(
      ...
      this.getAllAnnouncementData = reversedData;
      this.fillterAllAnnouncement = reversedData;
        
      
        console.log("view All Announcement", this.getAllAnnouncementData);
      },
      (error) => {
        // Handle error
      }
...
// view All Announcement end
// pagination and search for view Announcement start
AllAnnouncementperPage: number = 5;
currentAllAnnouncementPage: number = 1;

searchAllAnnouncement: string = '';


FilterAllAnnouncement() {
  this.fillterAllAnnouncement = this.getAllAnnouncementData.filter((item: { titleOfAnnouncement: string; date: string  }) =>
    item.titleOfAnnouncement.toLowerCase().includes(this.searchAllAnnouncement.toLowerCase()) ||
    item.date.toLowerCase().includes(this.searchAllAnnouncement.toLowerCase()) 
  );
}

getPaginatedAllAnnouncement(): any[] {
  const startIndex = (this.currentAllAnnouncementPage - 1) * this.AllAnnouncementperPage;
  const endIndex = startIndex + this.AllAnnouncementperPage;
  return this.fillterAllAnnouncement.slice(startIndex, endIndex);
}

previousAllAnnouncementPage(): void {
  if (this.currentAllAnnouncementPage > 1) {
    this.currentAllAnnouncementPage--;
  }
}

getPageNumbersAllAnnouncement(): number[] {
  const totalPages = Math.ceil(
    this.fillterAllAnnouncement.length / this.AllAnnouncementperPage
  );
  return Array.from({ length: totalPages }, (_, index) => index + 1);
}

changePageAllAnnouncement(pageNumber: number): void {
  if (pageNumber >= 1 && pageNumber <= this.getTotalPagesAllAnnouncement()) {
    this.currentAllAnnouncementPage = pageNumber;
  }
}


nextPageAllAnnouncement(): void {
  const totalPages = Math.ceil(
    this.fillterAllAnnouncement.length / this.AllAnnouncementperPage
  );
  if (this.currentAllAnnouncementPage < totalPages) {
    this.currentAllAnnouncementPage++;
  }
}


getTotalPagesAllAnnouncement(): number {
  return Math.ceil(
    this.fillterAllAnnouncement.length / this.AllAnnouncementperPage
  );
}
// pagination and search for view Announcement end


// for update Announcement start
selectedAnnouncement: any;
editAnnouncement(itemId: number) {
  // Find the selected item by its id
  this.selectedAnnouncement = this.fillterAllAnnouncement.find((item: { id: number; }) => item.id === itemId);
  console.log("Announcement id", itemId);
  console.log("Announcement select", this.selectedAnnouncement);
  // Show the 'editShiftTime' content
  this.loginService.showTable('editAnnouncement');
}

 UpdateAnnouncement(updateAnnouncementData: any) {
  this.testService.updateAnnouncement(updateAnnouncementData).subscribe(
    (response: any) => {

      console.log("updated Announcement", response);
      Swal.fire({
        title: 'Updated!',
 ...
 // for update Announcement end

 // for delete Announcement start
 ...
 deleteAnnouncement(id: any): void {
   ...
   Swal.fire({
     ...
     if (result.isConfirmed) {
       this.testService.deleteAnnouncement(id).subscribe(
         () => {
 ...
 // for delete Announcement end
 TodayAnnouncementData: any;
 getAnnouncementForToday() {
  
  // if ('/viewTodayPresent' === this.router.url) {

    // Call the service method to fetch all attendance data
    this.testService.getAnnouncementForToday().subscribe(
      (response: any) => {
       
        this.TodayAnnouncementData = response;
     
        console.log("today Announcement", response);
      },
      error => {
        Swal.fire('Error', error.error, 'error');
        // Hide the table if an error occurs
